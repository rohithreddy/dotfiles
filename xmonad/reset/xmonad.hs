...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    , ((modMask .|. shiftMask,   xK_Return), spawn "lxterminal")
    ...
    , ((modMask,               xK_c     ), runOrRaise
        "google-chrome"(className =? "Chrome"))  
    ...
    -- Restart xmonad
    , ((modMask              , xK_q     ),
        spawn "killall conky dzen2 gnome-settings-daemon nm-applet synapse trayer compton gnome-keyring-daemon && xmonad --restart")
...
appFontXft :: String
appFontXft = "xft:Droid Sans:pixelsize=12" 
...
myStartupHook = do
		...
		spawn "/usr/lib/gnome-settings-daemon/gnome-fallback-media-keys-helper"
		spawn "/usr/lib/gnome-settings-daemon/gnome-fallback-background-helper"
		spawn "/usr/bin/nm-applet"
		spawn "/usr/bin/synapse"
		spawn "/usr/bin/start-pulseaudio-x11"
		spawn "/usr/bin/blueman-applet"
		spawn "/usr/bin/gsettings-data-convert"
		spawn "/usr/bin/xdg-user-dirs-gtk-update"
		spawn "feh --bg-center /home/roh/Pictures/wallpaper"
		spawn "/usr/bin/trayer --edge top --align right --SetDockType true --SetPartialStrut true --expand true --width 10 --transparent true --height 22"
...
myDzenStatus = "dzen2 -w '1080' -ta 'l'" ++ myDzenStyle
myDzenConky  = "conky -c ~/.xmonad/conkyrc | dzen2 -x '1080' -w '700' -ta 'r'" ++ myDzenStyle
myDzenStyle  = " -h '22' -fg '#777777' -bg '#222222' -fn 'arial:bold:size=11'"
